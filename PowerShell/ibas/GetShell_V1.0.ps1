...
**** Date:20161127
**** Reason:Auto get & flush the ibas shell's function packages and dlls.
...
function GetShell()
{
    try
    {
        ...
        $StreamReader.Close()
        $StreamReader.Close()
        $Index = $RetString.IndexOf("<A HREF=""/ibas/shell/" + $FileName + $CureentDate)
        if ($Index -ge 0)
        {
            $Index += ("<A HREF=""/ibas/shell/" + $FileName + $CureentDate).Length;
